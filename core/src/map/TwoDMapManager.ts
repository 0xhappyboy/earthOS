...
import { PointData, LineData } from "../components/CoordinatePickingDataPanel";
import { LayerFeature } from "../components/LayersPanel";
import { DrawToolManager, CircleDrawTool, RectangleDrawTool, TriangleDrawTool, FreehandDrawTool, EllipseDrawTool, MarkerDrawTool, TextDrawTool, ArrowDrawTool, LineDrawTool, BezierDrawTool, SectorDrawTool, DrawToolType } from "../draw";
import { ImageDrawTool } from "../draw/ImageDrawTool";
import { Translations, Locale, getTranslation } from "../i18n";
import { LayerManager, CircleDrawLayer, RectangleDrawLayer, TriangleDrawLayer, DistanceMeasurementLayer, AreaMeasurementLayer, FreehandDrawLayer, EllipseDrawLayer, MarkerDrawLayer, TextDrawLayer, ArrowDrawLayer, LineDrawLayer, BezierDrawLayer, SectorDrawLayer, PointCoordinatePickLayer, MarkerLayer, BarChartLayer, CircleLayer, ClusterLayer, GeoJSONLayer, HeatmapLayer, PolygonLayer, PolylineLayer } from "../layers";
...
import { BasemapTypeEnum, CircleDrawData, CoordinateSystemTypeEnum, RectangleDrawData, TriangleDrawData, PolygonData, LayerInfo } from "../types";
...
export interface TwoDMapManagerCallbacks {
...
export class TwoDMapManager {
    ...
    private callbacks: TwoDMapManagerCallbacks;
    ...
    constructor(
        ...
        callbacks: TwoDMapManagerCallbacks,
        baseMapUrl?: string,
    ...
    private initEventManager(): void {
        ...
        this.eventManager.setMapView(this.map);
    ...
    private initLayers(): void {
        ...
        this.circleDrawLayer.setView(this.map);
        ...
        this.rectangleDrawLayer.setView(this.map);
        ...
        this.pointCoordinatePickLayer.setView(this.map);
        ...
        this.lineCoordinatePickLayer.setView(this.map);
        ...
        this.polygonCoordinatePickLayer.setView(this.map);
        ...
        this.freehandDrawLayer.setView(this.map);
        ...
        this.ellipseDrawLayer.setView(this.map);
        ...
        this.arrowDrawLayer.setView(this.map);
        ...
        this.distanceMeasureLayer.setView(this.map);
        ...
        this.sectorDrawLayer.setView(this.map);
    ...
    private bindEvents(): void {
        this.view.on("moveend" as any, () => {
            if (this.callbacks.onMoveEnd && !this.isDestroyed) {
                this.callbacks.onMoveEnd(this.getCenter(), this.getZoom());
            }
            this.updateScale();
        });
        if (this.callbacks.onMapClick) {
            this.map.on("click", (event: any) => {
                const [lng, lat] = this.map.getCoordinateFromPixel(event.pixel);
                this.callbacks.onMapClick?.({ longitude: lng, latitude: lat });
            });
        }
        this.map.on("click", () => {
            if (this.imageDrawLayer?.isInputActive) return;
        ...
        this.container.addEventListener("contextmenu", (e) => {
            ...
            if (circle && this.circleDrawLayer) {
                ...
                if (data) {
                    ...
                    this.showFloatingToolbarForCircle({ x, y }, data);
                    return;
                }
            }
            ...
            if (rectFeature && this.rectangleDrawLayer) {
                ...
                if (data) {
                    this.selectedRectangleId = data.id;
                    this.showFloatingToolbarForRectangle({ x, y }, data);
                    return;
                }
            }
            ...
            if (tri && this.triangleDrawLayer) {
                ...
                if (data) {
                    this.selectedTriangleId = data.id;
                    this.showFloatingToolbarForTriangle({ x, y }, data);
                    return;
                }
            }
            ...
            if (ellipse && this.ellipseDrawLayer) {
                ...
                if (data) {
                    this.selectedEllipseId = data.id;
                    this.showFloatingToolbarForEllipse({ x, y }, data);
                    return;
                }
            }
            ...
            if (marker && this.markerDrawLayer) {
                ...
                if (data) {
                    this.selectedMarkerId = data.id;
                    this.showFloatingToolbarForMarker({ x, y }, data);
                    return;
                }
            }
            ...
            if (arrow && this.arrowDrawLayer) {
                ...
                if (data) {
                    this.selectedArrowId = data.id;
                    this.showFloatingToolbarForArrow({ x, y }, data);
                    return;
                }
            }
    ...
    public handleTogglePopup(popup: any): void { }
    ...
    private onDrawingStart(type: DrawToolType): void {
        let msg = "";
        switch (type) {
            case DrawToolType.CIRCLE: msg = this.t.drawingCircle; break;
            case DrawToolType.RECTANGLE: msg = this.t.drawingRectangle; break;
            case DrawToolType.TRIANGLE: msg = this.t.drawingTriangle; break;
            case DrawToolType.FREEHAND: msg = this.t.drawingFreehand; break;
            case DrawToolType.FREEHAND_POLYGON: msg = this.t.drawingFreehandPolygon; break;
            case DrawToolType.ELLIPSE: msg = this.t.drawingEllipse; break;
            case DrawToolType.MARKER: msg = this.t.addingMarker; break;
            case DrawToolType.TEXT: msg = this.t.addingText; break;
            case DrawToolType.ARROW: msg = this.t.drawingArrow; break;
            case DrawToolType.LINE: msg = this.t.drawingLine; break;
            case DrawToolType.BEZIER: msg = this.t.drawingBezier; break;
            case DrawToolType.SECTOR: msg = this.t.drawingSector; break;
            default: msg = this.t.drawing;
        }
    ...
    public getZoom(): number { return this.view.getZoom() || 0; }
    ...
    public setZoom(zoom: number): void { this.view.setZoom(zoom); this.updateScale(); }
    ...
    public getBasemap(): BasemapTypeEnum | null { return this.currentBasemap; }
    ...
    public startEditShape(): void {
        ...
        this.showToast(this.t.noCirclesToEdit);
    }
    ...
    public locateUser(): void {
        ...
        navigator.geolocation.getCurrentPosition(
            (pos) => { this.setCenter([pos.coords.longitude, pos.coords.latitude]); this.setZoom(15); },
            () => { }
        );
    }

    public showCoordinateList(): void {
    ...
    public getPointDataForPanel(): PointData[] {
    ...
    public getLineDataForPanel(): LineData[] {
    ...
    public getPolygonDataForPanel(): import("../components/CoordinatePickingDataPanel").PolygonData[] {
    ...
    public locateToPoint(longitude: number, latitude: number): void {
        this.setCenter([longitude, latitude]);
        this.setZoom(18);
        this.showToast(`${this.t.locatedToPoint}: ${longitude.toFixed(6)}, ${latitude.toFixed(6)}`);
    }

    public locateToLine(points: { longitude: number; latitude: number }[]): void {
    ...
    public locateToPolygon(points: { longitude: number; latitude: number }[]): void {
        this.locateToLine(points);
        this.showToast(`${this.t.locatedToPolygon} ${points.length} ${this.t.points}`);
    }

    public getLayerFeatures(layerId: string): LayerFeature[] {
    ...
    public locateFeature(layerId: string, featureId: string): void {
        const layer = this.layerManager.getLayer(layerId);
    ...
    public copyFeatureCoordinates(layerId: string, featureId: string): void {
        const layer = this.layerManager.getLayer(layerId);
        let text = "";
    ...
    public updateLocale(locale: Locale): void {
        ...
        if (this.uiManager) {
            this.uiManager.updateLocale(this.t);
        }
    }

    public destroyMapOnly(): void {
